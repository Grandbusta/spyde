...
test("pdfkit: page size for A4 and for explicit points", () => {
...
test("pdfkit: line height is size * lineHeight, not the font's natural height", () => {
...
test("pdfkit: bounded width wraps and reports the widest line", () => {
  const r = new PdfKitRenderer();
...
test("pdfkit: unbounded width measures without wrapping", () => {
  const r = new PdfKitRenderer();
...
test("pdfkit: drawing into a box exactly as wide as the measured text does not wrap", () => {
  const r = new PdfKitRenderer();
...
test("pdfkit: text taller than the page does not add a page (D1)", async () => {
...
test("pdfkit: unknown font is a clear error at measure time (D2)", () => {
  const r = new PdfKitRenderer();
  assert.throws(
...
test("pdfkit: every built-in font measures without a font file", () => {
  const r = new PdfKitRenderer();
